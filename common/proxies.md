...
- **UUPS storage discipline.** New state in an upgradeable implementation must respect the reserved `__gap` (see [protocol version](/common/protocol-version.md)); shifting the layout corrupts every existing proxy.
...
- [Protocol version](/common/protocol-version.md) — the storage-gap discipline upgradeable proxies rely on.
