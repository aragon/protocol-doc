...
- **The executor.** It performs arbitrary on-chain actions on behalf of the organization, see [Actions and execution](/core/execution.md). This is how a passed proposal actually *does* something.
- **Its own permission database.** `DAO` inherits [`PermissionManager`](/core/permissions.md), so the rules for who may do what live in the DAO contract's own storage. A DAO is self-authorizing.
- **A signer.** It can validate signatures on the organization's behalf via [EIP-1271](/core/signature-validation.md).
...
`initialize` grants `ROOT_PERMISSION_ID` to `_initialOwner`. **ROOT is god-mode** (see [Permissions](/core/permissions.md)): whoever holds it can grant or revoke any permission. A freshly initialized DAO therefore has its initial owner in total control, which is fine as a *bootstrap* step and dangerous as an *end state*.
...
Each of the DAO's own privileged functions is gated by a permission (via the `auth` modifier from [`PermissionManager`](/core/permissions.md)):

| Permission | Gates |
|---|---|
| `EXECUTE_PERMISSION_ID` | `execute()`, see [Actions and execution](/core/execution.md). The most consequential permission in the protocol. |
...
Assets *leave* the DAO only through [`execute`](/core/execution.md), which is permission-gated. So funding is open; spending is governed.
...
`setMetadata` takes an opaque `bytes` blob (conventionally an IPFS CID) and only *emits* it as the `MetadataSet` event, it is not stored on-chain, so reading a DAO's current metadata means indexing that event. `daoURI()` implements EIP-4824 for standardized off-chain DAO discovery. See [DAO metadata](/core/dao-metadata.md) for the JSON shape and the `daoURI`-vs-blob distinction.
...
- [Permissions](/core/permissions.md), the authorization model the DAO inherits.
- [Actions and execution](/core/execution.md), how the DAO acts on the world.
