...
kind: contract
source: lock-to-vote-plugin/src/LockToVotePlugin.sol
summary: ""
---
...
Checks if an account can participate on a proposal. This can fail because the vote - has not started, - has ended, - was executed, or - the voter doesn't have voting powers. - the voter can increase the amount of tokens assigned
...
| `_proposalId` | `uint256` |  |
| `_voter` | `address` |  |
| `_voteOption` | [`IMajorityVoting.VoteOption`](#voteoption) |  |
...
Reverts if the vote cannot be cleared due to the voting settings. This can be because: - The plugin is in Standard votingMode and the voter has votes registered on active proposals
...
| `_actions` | `Action[]` |  |
| `_startDate` | `uint64` |  |
| `_endDate` | `uint64` | UNUSED: The parameter is kept for compatibility with IProposal but its value is computed internally. It should be set to 0. |
| `_data` | `bytes` |  |
...
Returns the current token supply. NOTE: It includes any non circulating supply that might be vesting, locked or undistributed.
...
| Returns | Type | Description |
| --- | --- | --- |
| `[0]` | [`IMajorityVoting.VoteEntry`](#voteentry) | The vote option cast by a voter for a certain proposal. |
...
| Returns | Type | Description |
| --- | --- | --- |
| `[0]` | [`MajorityVotingBase.VotingSettings`](#votingsettings) |  |
...
| `_lockManager` | [`ILockManager`](./ILockManager.md) |  |
| `_votingSettings` | [`MajorityVotingBase.VotingSettings`](#votingsettings) | The voting settings. |
| `_targetConfig` | [`IPlugin.TargetConfig`](#targetconfig) | Configuration for the execution target, specifying the target address and operation type (either `Call` or `DelegateCall`). Defined by `TargetConfig` in the `IPlugin` interface, part of the `osx-commons-contracts` package, added in build 3. |
...
| Returns | Type | Description |
| --- | --- | --- |
| `[0]` | `bool` | True if the proposal is executed or past the endDate, false otherwise. False if it doesn't exist. |
...
| Returns | Type | Description |
| --- | --- | --- |
| `[0]` | `bool` | True if the proposal is open, false if it hasn't started yet or if it has been already settled. |
...
Defines of the IExecutor contract where executable actions will be sent to.
...
| Parameter | Type | Description |
| --- | --- | --- |
| `_targetConfig` | [`IPlugin.TargetConfig`](#targetconfig) | The target Config containing the executor address and operation mode. |
...
```solidity
function usedVotingPower(uint256 _proposalId, address _voter) external view returns (uint256)
```
...
| Returns | Type | Description |
| --- | --- | --- |
| `[0]` | `uint256` | The amount of balance that has been allocated to the proposal by the given account. |
...
> **Dev:** `voteOption`, 1 -> abstain, 2 -> yes, 3 -> novotingPower updates any prior voting power, it does not add to the existing amount.
...
Thrown when a user's vote has been cleared.

| Parameter | Type | Description |
| --- | --- | --- |
| `proposalId` | `uint256` | the ID of the proposal where the vote was cleared. |
| `voter` | `address` | The address of the token holder whose vote was cleared. |
...
Thrown when a proposal action is targeting address(0) or the LockManager

| Parameter | Type | Description |
| --- | --- | --- |
| `actionIdx` | `uint256` | The index of the action |
| `target` | `address` | The invalid address that was passed |
...
Thrown when atempting to set the plugin or the LockManager as execution targets
...
### VoteRemovalForbidden

```solidity
error VoteRemovalForbidden(uint256 proposalId, address voter)
```

Thrown when attempting to remove a vote, because it is still active or because the plugin mode doesn't allow it.
...
### VoteRemovalUnauthorized

```solidity
error VoteRemovalUnauthorized(address caller)
```

Thrown when attempting to call clearVote() from an address other than the LockManager.

| Parameter | Type | Description |
| --- | --- | --- |
| `caller` | `address` | The address calling clearVote(). |
...
_Inherited from `MajorityVotingBase`._
...
Value: `0x28959d3c8d9c85fe68b3927a0425322ccc2f64692cea992da81e150c47353017`

The ID of the permission required to call `vote` and `clearVote`.
...
| Field | Type | Description |
| --- | --- | --- |
| `votingMode` | [`MajorityVotingBase.VotingMode`](#votingmode) | A parameter to select the vote mode. In standard mode (0), the voting power can be increased but votes cannot be replaced. In vote replacement mode (1), voters can change their vote multiple times and only the latest vote option is tallied. |
| `supportThresholdRatio` | `uint32` | The support threshold ratio. Its value has to be in the interval [0, 10^6) defined by `RATIO_BASE = 10**6`. This is intended as the primary metric for proposals to pass. |
