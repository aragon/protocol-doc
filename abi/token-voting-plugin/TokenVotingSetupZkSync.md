...
**Contract** · [`src/TokenVotingSetupZkSync.sol`](https://github.com/aragon/token-voting-plugin/blob/e97b783d76872d694f41dfc4bc846405019ca741/src/TokenVotingSetupZkSync.sol)
...
```solidity
constructor(TokenVoting _tokenVotingBase)
```
...
function decodeInstallationParameters(
    bytes _data
) external pure returns (MajorityVotingBase.VotingSettings votingSettings, TokenVotingSetupZkSync.TokenSettings tokenSettings, GovernanceERC20.MintSettings mintSettings, IPlugin.TargetConfig targetConfig, uint256 minApprovals, bytes pluginMetadata, address[] excludedAccounts)
```
...
function encodeInstallationParameters(
    ...
    TokenVotingSetupZkSync.TokenSettings tokenSettings,
...
| `name` | `string` | The name of the token, relevant only if a new token is deployed (i.e., `addr` is `address(0)`). |
| `symbol` | `string` | The symbol of the token, relevant only if a new token is deployed (i.e., `addr` is `address(0)`). |
