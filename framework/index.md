...
- **[Plugin setup](./plugin-setup.md)** — assembles **one plugin installation** (deploys the plugin + declares its permissions). A "factory" only in the sense of producing a single install.
- **[DAOFactory](./dao-factory.md)** — deploys **one DAO** (optionally with plugins) in a transaction.
- **PluginRepoFactory** (see [PluginRepo](./plugin-repo.md)) — deploys **one plugin repo**.
