...
- **How is a plugin published and versioned?** → the [PluginRepo](/framework/plugin-repo.md): one per plugin, storing every version as an immutable `release.build` tag pointing at a setup contract.
- **How does a DAO install a specific version, wiring exactly the right permissions, without handing anyone unchecked control?** → the [plugin setup](/framework/plugin-setup.md) contract (which declares what to deploy and which permissions to grant) plus the [PluginSetupProcessor](/framework/plugin-setup-processor.md) (which applies it under a two-step, governance-reviewable flow).
- **How does the ecosystem discover and trust DAOs and plugin repos?** → the [registries](/framework/registries.md) and their ENS names.

The rest of this section is those pieces. If you're building a plugin, the reading order is: this page → [choosing a base](/framework/plugin-types.md) → [plugin setup](/framework/plugin-setup.md) → [PluginRepo](/framework/plugin-repo.md) → [PSP](/framework/plugin-setup-processor.md). If you're just installing plugins onto a DAO, [PSP](/framework/plugin-setup-processor.md) and [DAOFactory](/framework/dao-factory.md) are the pages you want.

## What every plugin shares

All plugins implement `IPlugin`, which declares one thing they must answer, `pluginType()`, telling the framework how the plugin is deployed (see [choosing a base](/framework/plugin-types.md)):

```solidity
enum PluginType { UUPS, Cloneable, Constructable }
```

You never implement `IPlugin` directly; you extend one of the three [plugin base contracts](/framework/plugin-types.md), which give you the DAO reference, the [`auth`](/common/auth.md) modifier, ERC-165 support, and the execution helpers below.
...
- [Choosing a plugin base](/framework/plugin-types.md) — Plugin vs Cloneable vs UUPS.
- [Plugin setup](/framework/plugin-setup.md) — how a plugin declares its installation.
- [PluginSetupProcessor](/framework/plugin-setup-processor.md) — the install/update/uninstall engine.
