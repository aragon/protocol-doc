...
- **Secrets management**, kept out of the repo. Optionally integrates with [`vars`](https://github.com/vars-cli/vars), a CLI that keeps a central encrypted store and resolves, routes, and populates secrets into environment variables (so one vault serves every project); a plain `.env` file is the zero-install fallback.
