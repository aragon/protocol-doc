...
function defBlock(n: AstNode, bi: BuildInfo, what: "enum" | "struct", path: string): string[] {
  ...
  const label = what === "enum" ? "Option" : "Field";
  if (members.some((m) => ns.params?.[m.name])) {
    out.push(
      "",
      `| ${label} | ${what === "struct" ? "Type | " : ""}Description |`,
      `| --- | ${what === "struct" ? "--- | " : ""}--- |`,
      ...members.map((m, i) =>
        what === "struct"
          ? `| \`${m.name}\` | \`${m.type}\` | ${cell(ns.params?.[m.name])} |`
          : `| \`${m.name}\` (${i}) | ${cell(ns.params?.[m.name])} |`
      ),
    );
  }
  return out;
...
if (import.meta.main) {
  ...
  for (const [root, files] of [...groups].sort()) {
    ...
    for (const p of pages) await Deno.writeTextFile(join(dir, p.file), p.render(linkable));
