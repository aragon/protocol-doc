...
A **budget** answers the first half of a [Capital Router](../capital-router.md) flow: *how much is available to move right now?* A [strategy](./strategies.md) reads it, then (if it distributes) hands the total to a [splitter](./splitters.md). Budgets hold no state beyond their configuration (except the streaming one), the amount is computed fresh each call.

| Budget | Flow | Answers |
|---|---|---|
| **Full** | [dispatch](./dispatch-vs-request.md) | 100% of the vault's current balance of the token, recomputed each call |
...
| **Fixed** | [request](./dispatch-vs-request.md) | a constant amount, regardless of vault state (request-only, since a push couldn't trust the balance to be there) |
...
- [Splitters](./splitters.md) — the "who gets what" half a transfer strategy pairs with a budget.
- [Strategies](./strategies.md) and [dispatch vs. request](./dispatch-vs-request.md).
