...
A **splitter** answers the second half of a distributing [Capital Router](../capital-router.md) flow: given a total (from a [budget](./budgets.md)), *who gets what share?* A [dispatch](./dispatch-vs-request.md) splitter returns all recipients' amounts at once (`allocations(total)`); a request splitter returns one caller's share (`allocation(caller, total, data)`). Only *transfer* strategies use a splitter, swaps and burns have a single implicit destination.
...
- [Budgets](./budgets.md) — the total a splitter divides.
- [Ratio](../../common/ratio.md) — the OSx-wide `RATIO_BASE` convention (which rounds the other way).
- [Strategies](./strategies.md), [the plugins](./plugins.md).
