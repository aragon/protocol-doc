...
- **Voting power is read live from this balance.** Lock more and your weight goes up immediately, even on proposals already open (subject to the [mode rules](/plugins/lock-to-vote-plugin.md#the-voting-model-and-how-it-differs)).

## Unlocking: the crux

`unlock()` returns your tokens, but it is **gated** and **all-or-nothing** (it releases your *entire* locked balance, there is no partial unlock). Whether you can unlock *early* (while you still have live votes) is the vault's two-regime question, driven by the plugin's [voting mode](/plugins/voting-modes.md). Before releasing, `unlock` walks every proposal the vault knows about and, for each one still open, enforces:
...
2. **Did you vote on it?** If you have voting power counted on a still-open proposal, what happens depends on the [voting mode](/plugins/voting-modes.md):
...
- [Lock to Vote Plugin](/plugins/lock-to-vote-plugin.md) — the plugin overview, voting model, and permissions.
- [Voting modes](/plugins/voting-modes.md) — why Standard vs Vote Replacement changes whether you can unlock.
