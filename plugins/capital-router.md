...
- A **[Budget](./capital-router/budgets.md)** answers *"how much is available?"* (the whole balance, a fixed amount, a streaming unlock).
- A **[Splitter](./capital-router/splitters.md)** answers *"who gets what share?"* (equal, by ratio, tiered, gauge-weighted).
- A **[Strategy](./capital-router/strategies.md)** composes a Budget with (optionally) a Splitter and turns the numbers into DAO actions (transfer, swap, burn).
...
[Dispatch vs. Request](./capital-router/dispatch-vs-request.md) is worth its own read: the split is baked into the interfaces (dispatch budgets/splitters take no caller argument; request ones take caller-specific `data`), and it determines idempotency and who moves the money.

## The pieces

- [The plugins](./capital-router/plugins.md) — `DispatcherPlugin` (push), `RequesterPlugin` (pull), and `DispatchHubPlugin` (fan a dispatch out across many dispatchers, even on other DAOs). Plus the failsafe map, sequential execution, and the permission table.
- [Dispatch vs. request](./capital-router/dispatch-vs-request.md) — the push/pull axis in depth.
- [Budgets](./capital-router/budgets.md) — Full, Required, Fixed, Streaming.
- [Splitters](./capital-router/splitters.md) — Solo, Equal, Ratio, Tiered, gauge-weighted.
- [Strategies](./capital-router/strategies.md) — transfer, epoch-gated, burn, and the oracle-protected swaps.
- [Oracles](./capital-router/oracles.md) — the price feeds the swap strategies use for slippage protection.
- [What you can build](./capital-router/use-cases.md) — the composition catalogue: payroll, vesting, DCA, buybacks, rewards, and the extension points.
...
- [Governance plugins](./index.md) — the vote-each-proposal counterpart Capital Router deliberately isn't.
