...
The setup deploys the plugin and returns the **exact permissions** the install needs. `prepareInstallation` decodes its install `data`, deploys the proxy, and lists the grants; `prepareUninstallation` lists the matching revokes. This is the ABI and the permission set that [Deploy your first DAO](/guides/deploy-a-dao.md) and [Install a plugin](/guides/install-a-plugin.md) feed to the PSP:
...
- `prepareUninstallation` mirrors `prepareInstallation`'s grants as revokes. For an **updatable** plugin, extend [`PluginUpgradeableSetup`](/framework/plugin-setup.md) and add `prepareUpdate` (see [Update a plugin](/guides/update-a-plugin.md)).

## Next

- [Publish a plugin to a PluginRepo](/guides/publish-a-plugin.md), turn this setup into an installable, versioned release.
- [Write a custom condition](/guides/manage-permissions.md) if your plugin's permissions need dynamic rules.
