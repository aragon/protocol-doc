...
As in [manage permissions](/guides/manage-permissions.md), the test uses a bare DAO where you hold `EXECUTE`, so you can `dao.execute` the apply batch directly; on a governed DAO those same actions are a [proposal](/guides/create-vote-execute.md).
...
- [Update a plugin](/guides/update-a-plugin.md), the same prepare/apply rhythm for moving an installed plugin to a new version.
- [Build a plugin](/guides/build-a-plugin.md), write the `PluginSetup` whose prepared permissions this flow applies.
